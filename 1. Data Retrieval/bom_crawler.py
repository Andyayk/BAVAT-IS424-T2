...
soup = BeautifulSoup(urlopen(current_url).read(), "html.parser") # generate list of links for the letter indices
letters = soup.findAll('a', href= re.compile('letter=')) # limit list here if needed but note that only 2 crawling files can be run simultaneously
print letters
...
for i in range(0,27): # loop through all letter indices for movies - adjust based on list length
	current_url=letter_index[i]
	soup = BeautifulSoup(urlopen(current_url).read(), "html.parser")
	...
	movies = soup.findAll('a', href= re.compile("/movies/\?id=."))
	for t in movies:
...
with open("bom_revenue_data.csv", "wb") as f:
	...
	for url in movie_links:
		if "elizabeth" in url and "elizabethtown" not in url: # fixes an annoying encoding error in an inelegant way
		...
		print url
		current_url = (url + "&adjust_yr=2019&p=.htm")  # do all movies in 2019 dollars (done automatically by site with correct URL)
